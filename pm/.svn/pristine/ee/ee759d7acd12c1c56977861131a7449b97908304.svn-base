...
@Service
@Transactional
public class INSCUserRoleServiceImpl extends BaseServiceImpl<INSCUserRole, String> implements INSCUserRoleService {
	@Resource
	...
	@Override
	protected BaseDao<INSCUserRole, String> getBaseDao() {
		return inscUserRoleDao;
	}

	@Override
	public List<String> selectRoleidByUserid(String userId) throws ServiceException {
		...
		if (roleId == null || "".equals(roleId)) {
			...
			for (INSCUserRole userRole : userRoleList) {
				...
				for (String rileId : roleids) {
					roleName = roleName + inscRoleDao.selectRoleNameById(rileId) + ",";
				}
				INSCUser user = inscUserDao.selectByUserId(userRole.getUserid());
				if (user != null) {
					if (roleName.length() > 0) {
						user.setRolenames(roleName.substring(0, roleName.length() - 1));
					}
					result.add(user);
				}
