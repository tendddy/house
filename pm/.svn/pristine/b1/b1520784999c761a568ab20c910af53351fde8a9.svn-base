require(["jquery", "bootstrapdatetimepicker", "bootstrapdatetimepickeri18n", "bootstrap-table", "bootstrap","bootstrapTableZhCn","jqvalidatei18n","additionalmethods","jqtreeview","zTree","zTreecheck"], function ($) {
	...
	$(function() {
		//平台列表查看
		$("#appCheckPolicy").bootstrapTable({
	        url: "initappchecklist",
	        method: 'get',
            ...
            columns: [
                    {field: 'state',align: 'center',valign: 'middle',checkbox: true},
      	            ...
      	            {field: 'platformStutas',title: '平台状态',align: 'center',valign: 'middle',sortable: true},
		...
		$("#appordersPolicy").bootstrapTable({
	        url: "initapporderslist",
	        method: 'get',
		...
		$("#detailquerybutton").click(function(){
			...
			window.location.href = "appdetailquery?id="+arrayuserid+"&querymonth="+$("#querymonth").val()+"&&querydate="+$("#querydate").val();
		})
		// 查询条件【重置】按钮
		...
		$("#toggle").on("click", function(e) {
			$('#appCheckPolicy').bootstrapTable('toggleView');
		});
		//刷新
		$("#refresh").on("click", function(e) {
			$('#appCheckPolicy').bootstrapTable('refresh');
		});
		//详细页面start*****************************************************
		// 详细查询页面【查询】按钮
		$("#querybtn").on("click", function(e) {
			...
			if($("#corpuserid").val()){
				postdata += "&corpuserid=" + $("#corpuserid").val();
			}
			if($("#orderid").val()){
		...
		$("#togglebtn").on("click", function(e) {
			$('#appordersPolicy').bootstrapTable('toggleView');
		});
		//详细查询页面 刷新
		$("#refreshbtn").on("click", function(e) {
			$('#appordersPolicy').bootstrapTable('refresh');
		});
		//详细查询页面 【返回】按钮
...
//详细查询页面start*****************************************************
//详细查询页面 查询列表
function reloaddataOfOrders(data){
	$.ajax({
		url : "initapporderslist",
		type : 'GET',
		dataType : "json",
		...
		success : function(data) { 
			$('#appordersPolicy').bootstrapTable('load', data);
		}
	});
}
//详细查询页面 得到查询参数
...
function getSelectedRows() {
    var data = $('#appCheckPolicy').bootstrapTable('getSelections');
    if(data.length == 0){
    	alert("至少选择一行数据！");
    }else{
...
function reloaddata(data){
	$.ajax({
		...
		success : function(data) { 
			$('#appCheckPolicy').bootstrapTable('load', data);
		}
	});
}
//应用平台对账页面 得到查询参数
