...
@Service
@Transactional
public class INSCScheduleServiceImpl extends BaseServiceImpl<INSCSchedule, String> implements INSCScheduleService {
	@Resource
	...
	@Override
	protected BaseDao<INSCSchedule, String> getBaseDao() {
	...
	public String executebyids(List<String> ids) {
		...
		if (ids != null && ids.size() > 0 && !ids.isEmpty()) {
			List<INSCSchedule> stateList = inscScheduleDao.getScheduleStateListByIds(ids);
			int i = 0, j = 0;
			Integer state = null;
			String ip = null;
			...
			if (j > 0) {
				msgsb.append("您选择的计划中有").append(j).append("个计划为非本地执行计划，请选择本地执行计划后再执行！");
				map.put("fail", msgsb.toString());
			}
	...
	private Date getNextExeDate(INSCSchedule schedule) {
		...
		if (schedule != null) {
			...
			if (iscron != null && period != null && StringUtils.isNotBlank(periodunit) && starttime != null) {
	...
	public Map<String, Object> getEditInfo(String scheduleId) {
		...
		List<INSCTask> taskList = inscTaskDao.selectAll();
