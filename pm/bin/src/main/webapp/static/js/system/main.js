		...
		function($) {
			$(function() {
				...
				$('.input-group').on(
						...
						'.form-control',
						function() {
							$(this).closest('.input-group, .form-group')
									.addClass('focus');
						}).on(
						'blur',
						'.form-control',
				...
				$("#appmenu a[data-bind]").on(
						...
						function(e) {
							if ($(this).attr("data-bind") != "") {
								...
								var ulname = "";
								var nodeName = "";
								var thistext = $(this).html().substring(
										$(this).html().indexOf("span>") + 5);
								if (allul.length > 0) {
									for (i = allul.length - 1; i >= 0; i--) {
										ulname = $(allul[i]).html();
										nodeName = nodeName
												+ ulname.substring(ulname
														.indexOf("span>") + 5,
														ulname.indexOf("<b"))
												+ ">>";
									}
								...
								var spanclass = $(this).find("span:first")
										.attr("class");
								$("#" + $(this).attr("target")).attr("src",
										clickurl);
								createlionbottom(clickurl, thistext, spanclass,
										nodeName + thistext);
							}
						});
...
/*******************************************************************************
 * openDialogForSelf 方法 对于CM相关页面需要的宽度和core.js的不同，重构了openDialog方法自定义宽度
 ******************************************************************************/
function openDialogForSelf(val,length) {
	$.xDialog({
		remote : val,
		width : length+"px"
	});
