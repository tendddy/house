require(["jquery","des","bootstrapdatetimepicker", "bootstrapdatetimepickeri18n", "bootstrap-table", "bootstrap","bootstrapTableZhCn","jqvalidatei18n","additionalmethods","jqtreeview","zTree","zTreecheck"], function ($) {
	...
	$(function() {
			...
			if(!$(this).hasClass('active')){
				...
				if (index == 1){
					...
					$.ajax({
						...
						type : 'POST',
						dataType : "json",
						data: "orderPayMessage="+strEncrypt("{'paymentId':"+$("#paymentId").val()+",'goods_name':'测试'}"),
						async : false,
						error : function() {
		...
		$("#cftPay").click(function(){
			var data = {"paymentId":$("#paymentId").val(),"goods_name":"测试"};
			var encoded = strEncrypt(JSON.stringify(data));
			var message = {"orderPayMessage":encoded};
			$.ajax({
				url : "gotoTenpayWap.do",
				type : 'POST',
